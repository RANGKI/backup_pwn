...
def find_syscall_ret_all_sections(binary_path):
    # Load the binary
    elf = pwn.context.binary = pwn.ELF(binary_path, checksec=False)
    
    # Byte pattern for syscall (0x0f 0x05) followed by ret (0xc3)
    pattern = b'\x0f\x05\xc3'
    ...
    for section in elf.sections:
        ...
        while True:
            ...
            print(f"Found 'syscall; ret' in section '{section_name}' at address: 0x{match_addr:x}")
            found = True
            offset += 1  # Move to the next byte to continue searching
    
    if not found:
        print("No 'syscall; ret' pattern found in any section of the binary.")

if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("Usage: python find_syscall_ret_all_sections.py <binary_path>")
        sys.exit(1)
    find_syscall_ret_all_sections(sys.argv[1])
