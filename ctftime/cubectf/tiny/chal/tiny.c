...
static ssize_t rio_read(rio_t *rp, char *usrbuf, size_t n) {
    ...
    while (rp->rio_cnt <= 0) {  /* Refill if buffer is empty */
        ...
        if (rp->rio_cnt == 0)   /* End-of-File */
            return 0;
        rp->rio_bufptr = rp->rio_buf;  /* Reset buffer pointer */
    }

    /* Copy the lesser of n or rp->rio_cnt bytes from internal buf to usrbuf */
    cnt = (rp->rio_cnt < (int)n) ? rp->rio_cnt : n;
...
void url_decode(char* src, char* dest, int len) {
    char *p = src;
    char code[3] = { 0 };
    while(--len) {
        if(*p == '%') {
...
void parse_request(int fd, http_request *req){
    ...
    // Extract method
    while (buf[i] && !isspace(buf[i]) && i < 15) {
        method[i] = buf[i];
        i++;
    }
    method[i] = '\0';
    ...
    url_decode(filename, req->filename, j);
}
...
off_t offset = req->offset; // Start from the correct position
lseek(in_fd, offset, SEEK_SET); // Ensure we start reading from `req->offset`

while (offset < req->end) {
    ssize_t bytes_to_read = (req->end - offset) < BUF_SIZE ? (req->end - offset) : BUF_SIZE;

    bytes_read = read(in_fd, buffer, bytes_to_read);
    if (bytes_read <= 0) {
        perror("read failed");
        break;
    }
    ...
    offset += bytes_written; // Update offset based on actual bytes written
}
...
void crash_handler(int sig) {
    printf("Caught signal %d. A thread crashed!\n", sig);
    // Optionally restart thread logic here
}

int main(int argc, char** argv){
   signal(SIGSEGV, crash_handler);  // Catch crashes in each thread
